...
export MAX_IT="100 200 300 400 500 600 700 800 900 1000"
...
for n_threads in $NUM_THREADS; do
    ...
    for VERSION in $VERSIONS; do
        for S in $MAX_IT; do
            echo "Starting run using $VERSION with grid size $S and $n_threads threads"
            #OMP_NUM_THREADS=$n_threads ./$EXECUTABLE 200 $S $DEFAULT_T 0 0 1 > $OUTDIR/Jacobi_N${S}_${n_threads}_v1.txt
            ./$EXECUTABLE $DEFAULT_N $S $DEFAULT_T 0 0 $VERSION > $OUTDIR/Jacobi_N${S}_${n_threads}_v${VERSION}.txt
        done
    done
done
for n_threads in $NUM_THREADS; do
        for S in $MAX_IT; do
            echo "Starting run using $VERSION with grid size $S and $n_threads threads"
            OMP_NUM_THREADS=$n_threads ./$EXECUTABLE $DEFAULT_N $S $DEFAULT_T 0 0 1 > $OUTDIR/Jacobi_N${S}_${n_threads}_v1.txt
            #./$EXECUTABLE $S $DEFAULT_MAX_IT $DEFAULT_T 0 0 $VERSION > $OUTDIR/Jacobi_N${S}_${n_threads}_v${VERSION}.txt
        done
    done
...
#make clean
#make
#export SIZES2="100 200 300 400 500"
#for S in $SIZES2; do
 #   for n_threads in $NUM_THREADS; do
