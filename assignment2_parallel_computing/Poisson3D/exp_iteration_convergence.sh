...
echo "tol = 0.001" >> $EXP_DIR/setup.txt
...
for n in $Ns; do
    echo "N = $n"
    ./poisson_gs $n $max_iters 0.001 $start_T 0 > $OUT_DIR/output_gs_N_${n}.txt
    ./poisson_j $n $max_iters 0.001 $start_T 0 > $OUT_DIR/output_j_N_${n}.txt
done;echo
